...
boolean  mqtt_start(void* ctx)
{
        ...
        connectParams.disconnectHandler = disconnectCallbackHandler;
              ...
              /***********************************************************************************************************************************************************/
              rc = subscribe_MQTT(MQTTcallbackHandler, "dhairya");
              /***********************************************************************************************************************************************************/
              delay(1000);
...
int subscribe_MQTT(int32_t (*callbackHandler)(MQTTCallbackParams), char * sTopic) {
  ...
  Serial.println("Ok");
  /********************************************************************************************************/
  Serial.println(subParams.pTopic);
  /**********************************************************************************************************/
  Serial.flush();
  return rc;
}
...
//edited by dhairya
    sprintf(cPayload, "{\"%s\": \"%d\", \"%s\": \"%lld\", \"%s\": %d, \"%s\": %d, \"%s\": %d}","DeviceID", serial, "TimeStamp", (long long)loopCount, "Temperature", loopCount, "LUX", loopCount, "Humidity", loopCount);
    loopCount++;
    
    Serial.println("publish_MQTT go");
    /*****************************************************************************************************************************************/
    rc = publish_MQTT("dhairya", cPayload);
    /*****************************************************************************************************************************************/
    Serial.flush();
}
